...
$branches = $conn->query("SELECT Branch_Id, Branch_Name FROM Branch_Master WHERE Branch_Status = 1");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['cus_photo']) && $_FILES['cus_photo']['error'] === 0) {
        $photo_path = 'uploads/photos/' . time() . '_' . basename($_FILES['cus_photo']['name']);
        move_uploaded_file($_FILES['cus_photo']['tmp_name'], $photo_path);
    }

    if (isset($_FILES['cus_id_copy']) && $_FILES['cus_id_copy']['error'] === 0) {
        $id_copy_path = 'uploads/idcopies/' . time() . '_' . basename($_FILES['cus_id_copy']['name']);
...
<div class="main">
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <select name="branch_id" required>
      ...
      <?php
        $res = $conn->query("SELECT Branch_Id, Branch_Name FROM Branch_Master");
        while ($b = $res->fetch_assoc()):
      ?>
        <option value="<?= $b['Branch_Id'] ?>" <?= isset($editData['Branch_Id']) && $editData['Branch_Id'] == $b['Branch_Id'] ? 'selected' : '' ?>><?= $b['Branch_Name'] ?></option>
      <?php endwhile; ?>
    </select>
